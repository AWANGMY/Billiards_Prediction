...
def parse_args():
    ...
    parser.add_argument('--processed-path',
                        default=os.path.join('Output', 'reproduction',
                                             'billiards_layout_paper40.pt'))
    parser.add_argument('--protocol',
                        choices=['paper40_clean'],
                        default='paper40_clean')
...
def train_one_epoch(model, loader, criterion, optimizer, device):
    ...
    return metric_dict(total_loss, total_correct, total_count, batch_accuracy_sum,
                       batch_count, loader.batch_size)
...
def run_task(data, model_name, task, splits, args, device):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if should_log:
            print(format_progress(model_name, task, epoch, args.epochs,
                                  final_train_metrics))
    ...
    return {
        ...
        'best_test_release_denominator_accuracy': final_test_metrics['release_denominator_accuracy'],
...
def format_progress(model_name, task, epoch, epochs, train_metrics):
    ...
    return (model_name + ' ' + task + ' epoch ' + str(epoch) + '/' + str(epochs) +
...
def write_summary(results, output_dir, protocol):
    ...
    summary_path = os.path.join(output_dir,
                                'OtherMethods_' + protocol + '_summary_' +
                                timestamp + '.csv')

    fieldnames = ['model', 'task', 'protocol', 'seed', 'epochs', 'weight_decay',
                  'reported_test_accuracy', 'paper_blcnn_target_accuracy',
                  'accuracy_gap_to_paper_blcnn', 'best_epoch',
                  'best_selection_accuracy', 'train_size', 'val_size',
