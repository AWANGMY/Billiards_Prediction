...
The potted label is both ordered and multimodal.  A pure ordinal head, motivated by CORAL \cite{cao2020coral}, trains binary thresholds $z_k=\mathbb{1}[y>k]$, but the observed distribution has peaks at 0, 7, and 8.  We therefore evaluate a combined ordinal-class head with cross-entropy plus a weighted ordinal term,
\[
  \mathcal{L}_{pot}
  = \mathrm{CE}(p,y) + \lambda_{ord}
    \sum_{k=0}^{8}\mathrm{BCE}(\ell_k, \mathbb{1}[y>k]),
\]
and a stronger joint-head variant.  The joint head predicts one of $2\times2\times10=40$ states:
\[
...
\]
This head explicitly models outcome correlation: a state favorable for potting many balls should also affect clear and win probabilities.  The selected joint variant uses
\[
...
All experiments use the same deterministic split protocol.  BLFormer is trained with AdamW, learning rate $3\times10^{-4}$, weight decay 0.01, batch size 64, cosine annealing, dropout 0.1, gradient clipping at 1.0, and early stopping with patience 60 over at most 400 epochs.  The model has two encoder layers, four attention heads, and hidden width $d\in\{64,80,88\}$.  The selected joint model uses width 80, feed-forward width 160, CLS+mean pooling, and 121,426 parameters.  We report accuracy for each task and mean accuracy; macro-F1 and potted-count MAE are used for error analysis.  Model selection balances the three validation accuracies instead of optimizing a single task.
...
Independent BLFormer, best mean & 69.42 & \textbf{70.45} & 65.81 & 68.56 \\
Joint Outcome BLFormer, width 80 & 70.96 & 70.19 & 66.92 & 69.36 \\
Joint Outcome BLFormer + auxiliary, width 80 & \textbf{71.82} & 69.67 & \textbf{67.10} & \textbf{69.53} \\
\bottomrule
\end{tabular}
\end{table}
...
\bibitem{zhang2024billiards}
Qianru Zhang, Zheng Wang, Cheng Long, and Siu-Ming Yiu.
\newblock Billiards Sports Analytics: Datasets and Tasks.
\newblock \emph{arXiv preprint arXiv:2407.19686}, 2024. \url{https://arxiv.org/abs/2407.19686}.
...
\newblock \emph{Pattern Recognition Letters}, 2020. \url{https://arxiv.org/abs/1901.07884}.
