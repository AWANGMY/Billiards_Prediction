...
for task in TASKS:
    ...
    model = MLP(hyperparameters=hyperparameters).to(device)
    ...
    checkpoint_path = os.path.join(output_dir, "MLP_" + task + ".pt")
    history_path = os.path.join(output_dir, "MLP_" + task + "_history.csv")
    summary_path = os.path.join(output_dir, "MLP_" + task + "_train.json")
...
summary_path = os.path.join(output_dir, "MLP_training_summary.json")
