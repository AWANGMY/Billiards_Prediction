...
def parse_args():
    ...
    parser.add_argument('--output-dir',
                        default=os.path.join('Output', 'reproduction'))
...
def load_processed_data(processed_path):
    ...
    if not os.path.exists(processed_path):
        raise FileNotFoundError(
            'Processed data not found: ' + processed_path +
...
def train_one_epoch(model, loader, criterion, optimizer, device, zero_grad):
    ...
    return metric_dict(total_loss, total_correct, total_count, batch_accuracy_sum, batch_count, loader.batch_size)
...
def metric_dict(total_loss, total_correct, total_count, batch_accuracy_sum, batch_count, batch_size):
    ...
    return {
        ...
        'batch_mean_accuracy': batch_accuracy_sum / batch_count if batch_count > 0 else None,
        'release_denominator_accuracy': (
            total_correct / release_denominator * 100.0 if release_denominator > 0 else None),
...
def run_task(data, task, splits, args, device, weight_decay):
    ...
    train_loader = make_loader(x, y, splits['train'], args.batch_size, True,
                               args.seed, args.num_workers)
    val_loader = make_loader(x, y, splits['val'], args.batch_size, False,
                             args.seed, args.num_workers)
    test_loader = make_loader(x, y, splits['test'], args.batch_size, False,
                              args.seed, args.num_workers)
    ...
    for epoch in range(1, args.epochs + 1):
        final_train_metrics = train_one_epoch(model, train_loader, criterion,
                                              optimizer, device, zero_grad)

        should_eval = (selection_loader is not None and
                       args.eval_every_epochs > 0 and
                       (epoch % args.eval_every_epochs == 0 or epoch == args.epochs))

        if should_eval:
            final_selection_metrics = evaluate(model, selection_loader, criterion, device)
            selection_accuracy = final_selection_metrics['accuracy']
            if best_selection_accuracy is None or selection_accuracy > best_selection_accuracy:
                best_selection_accuracy = selection_accuracy
                best_epoch = epoch
                best_state = clone_state_dict(model)

        print(format_progress(task, epoch, args.epochs,
                              final_train_metrics, final_selection_metrics, selection_name))
    ...
    if args.save_best_checkpoint and best_state is not None:
        checkpoint_path = os.path.join(args.output_dir,
                                       'BLCNN_' + args.protocol + '_' + task + '_best.pt')
    ...
    primary_test_metrics = best_test_metrics if best_state is not None else final_test_metrics
    ...
    return {
        ...
        'final_test_release_denominator_accuracy': final_test_metrics['release_denominator_accuracy'],
        ...
        'best_test_release_denominator_accuracy': best_test_metrics['release_denominator_accuracy'],
...
def format_progress(task, epoch, epochs, train_metrics, selection_metrics, selection_name):

    message = (task + ' epoch ' + str(epoch) + '/' + str(epochs) +
               ' train_acc=' + format_float(train_metrics['accuracy']))
    if selection_metrics is not None:
        message += ' ' + selection_name + '_acc=' + format_float(selection_metrics['accuracy'])
    return message
...
def write_task_result(result, output_dir):
    ...
    csv_path = os.path.join(output_dir,
                            'BLCNN_' + result['protocol'] + '_' + result['task'] + '_result.csv')
...
def write_summary(results, output_dir, protocol):
    ...
    summary_path = os.path.join(output_dir,
                                'BLCNN_' + protocol + '_summary_' + timestamp + '.csv')

    fieldnames = ['model', 'task', 'protocol', 'seed', 'epochs', 'weight_decay',
                  'reported_test_accuracy', 'paper_target_accuracy', 'accuracy_gap_to_paper',
                  'best_epoch', 'best_selection_accuracy', 'train_size', 'val_size',
...
def main():
    ...
    print('split_sizes:', {name: len(splits[name]) for name in ['train', 'val', 'test']})
